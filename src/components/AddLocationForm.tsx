...
export const AddLocationForm: React.FC<AddLocationFormProps> = ({
	...
	const performSearch = useCallback(async (query: string) => {
		...
		try {
			...
			toast.error(
				error.response?.data?.message || "Failed to search locations",
			);
			setSearchResults([]);
			setShowResults(false);
		} finally {
	...
	const handleSelectLocation = (location: Location) => {
		...
		setSearchQuery(
			`${location.unlocode} - ${location.place}, ${location.country}`,
		);
		setShowResults(false);
	};
	...
	useEffect(() => {
		if (buttonRef.current && selectedLocation) {
			console.log(
				"Button ref available, selectedLocation:",
		...
		<div className="relative z-0">
			<div className="space-y-4">
				<div className="relative z-10">
					<Input
						data-tour="locations-search-input"
				...
				{selectedLocation && (
					<div
						className="p-4 bg-blue-50 border-2 border-blue-200 rounded-lg relative"
						style={{ zIndex: 1 }}
					>
						<div className="flex items-center justify-between gap-4">
							...
							>
								<button
									data-tour="locations-add-selected"
									ref={buttonRef}
									type="button"
									onClick={(e) => {
										...
										console.log(
											"Calling handleAddLocation with unlocode:",
											unlocode,
										);
					...
					!isSearching && (
						<div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
							<p className="text-sm text-yellow-800">
								No locations found. Try a different search term or use a valid
								UN/LOCODE.
							</p>
						</div>
					)}
