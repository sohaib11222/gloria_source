...
import { clearSourceAuth, expiredSignInNotice } from "./authSession";
...
function getCurrentApiBaseUrl(): string {
	...
	if (import.meta.env.VITE_API_BASE_URL) {
		...
		if (
			...
			console.warn(
				"⚠️ Converting HTTP API URL to HTTPS to avoid mixed content:",
				envUrl,
			);
...
function redirectToLoginForUnauthorized(url?: string): void {
	...
	if (!currentPath.endsWith("/login") && !currentPath.endsWith("/login/")) {
		window.location.replace(import.meta.env.PROD ? "/source/login" : "/login");
	}
}
...
api.interceptors.response.use(
	(response) => {
		...
		if (
			response.config.url?.includes("/sources/import-branches") ||
			response.config.url?.includes("/sources/upload-branches")
		) {
		...
		// With validateStatus: () => true, 401 is delivered here; expired sessions must go to login.
		if (response.status === 401) {
			redirectToLoginForUnauthorized(response.config.url);
		}
	...
	(error) => {
		...
		if (error.response) {
			...
			// Handle 401 - redirect protected requests to login.
			if (status === 401) {
				redirectToLoginForUnauthorized(error.config?.url);
			}
